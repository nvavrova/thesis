...
import ast.node.expression.arithmetic.Arithmetic;
import ast.node.expression.atom.*;
import ast.node.expression.bitwise.Xor;
import ast.node.expression.display.*;
import ast.node.expression.logical.And;
import ast.node.expression.logical.Logical;
import ast.node.expression.logical.Not;
import ast.node.expression.logical.Or;
import ast.node.expression.maker.DictMaker;
import ast.node.expression.maker.SetMaker;
import ast.node.expression.primary.Subscript;
import ast.node.expression.primary.SubscriptIndex;
import ast.node.expression.primary.SubscriptSlice;
import ast.node.expression.unary.Invert;
import ast.node.expression.unary.Minus;
import ast.node.expression.unary.Plus;
import ast.node.param.Param;
import ast.node.param.Params;
import ast.node.param.TypedParam;
import ast.node.statement.Statement;
import ast.node.statement.compound.*;
import ast.node.statement.flow.*;
import ast.node.statement.simple.*;
...
public class AstBuilder {
	...
	private class AstBuilderVisitor implements Python3Visitor<Py3Node> {
		...
		public Py3Node visitAnd_expr(@NotNull Python3Parser.And_exprContext ctx) {
			...
			return new ast.node.expression.bitwise.And(this.getLocInfo(ctx), children);
		}
		...
		public Py3Node visitSubscript(@NotNull Python3Parser.SubscriptContext ctx) {
			...
			//      index=test | lowerBound=test? colon=':' upperBound=test? stride=sliceop?
			if (ctx.index != null) {
				this.indent--;
		...
		public Py3Node visitNumber(@NotNull Python3Parser.NumberContext ctx) {
			...
			if (ctx.integer() != null) {
				...
				this.indent--;
				return new ast.node.expression.atom.Float(this.getLocInfo(ctx), nr);
			}
